...
int main(int argc, char *argv[ ]) {
    ...
    vector<string> instances = {"./instancias_teste/P-n16-k8.txt","./instancias_teste/P-n19-k2.txt","./instancias_teste/P-n20-k2.txt",
    ...
    int great_sol_instances[] = {450,212,216,529,510,696,741,568};
        ...
        for (int it = 0; it < code_iterations; it++){
            ...
            if(type==0){
                ...
                if (arq_instances) {
                    ...
                    while (getline(arq_instances, buffer)) {
            ...
            if (type == 1)
            {
                ...
                temp_sum_meta[l] = accumulate(final_route.begin(), final_route.end(),0);
               
                temp_sum_constr[l] = accumulate(greedy_distance.begin(), greedy_distance.end(),0);
    ...
    //O CODIGO COMENTADO A BAIXO SERÁ USADO PARA CRIAÇÃO DA TABELA COM AS INSTANCIAS
/*
    if (type == 1)
    {
        ...
        {   
            ...
            gap_meta[i] = (((final_sum_meta[i]-great_sol_instances[i])/great_sol_instances[i])*100); 
            media_instance_meta[i] = sum_media_meta[i]/code_iterations;

            constr_time[i] = constr_time[i]/code_iterations;
            gap_constr[i] = (((final_sum_constr[i]-great_sol_instances[i])/great_sol_instances[i])*100);
        ...
        for (i = 0; i < n_files; i++)
        { 
            myfile << "INSTANCIA[" <<i<<"]:     " << great_sol_instances[i]<< "      " << media_instance_constr[i]<< "         " << final_sum_constr[i]<< "    " 
  ...
  */  
    ...
    for (m = 0; m < final_solution.size(); m++)
        ...
        cout <<endl<< "Distance Travelled: " << distance << endl;
